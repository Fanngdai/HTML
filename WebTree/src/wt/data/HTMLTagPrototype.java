...
public class HTMLTagPrototype implements Comparable<HTMLTagPrototype>, Cloneable {
    ...
    private ArrayList<String> legalParents;
...
//        // Adds the attribute to every object - Homework1 part3
        if(!(tagName.equalsIgnoreCase(TAG_HTML) || tagName.equalsIgnoreCase(TAG_HEAD) 
                || tagName.equalsIgnoreCase(TAG_TITLE) || tagName.equalsIgnoreCase(TAG_LINK)
                || tagName.equalsIgnoreCase(TAG_BODY))){
            attributes.put("id", "");
   ...
   /**
	...
	for (String attributeName : attributes.keySet()) {
	    clonedTag.addAttribute(attributeName, attributes.get(attributeName));
	}
